  ...
  return (
    ...
    <div className='grid grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 place-items-center gap-1 max-w-6xl mx-auto border'>
        {logoData.length > 0 ? logoData?.slice(currentIndex, currentIndex + 6 ).map((item, index)=>(
            <div key={index} className='border border-stone-700 w-[140px]  md:w-[180px] flex items-center justify-center '>
                {/* <img src={`https://tackletips.in/uploads/brands/logo/${item?.brand_image}`} alt="img" className='w-[150px] object-cover' /> */}
                <img src={`https://tackletips.in/uploads/brands/logo/${item.brand_image}`} alt="img" className='w-[100px] md:w-[150px] h-[80px] md:h-[100px] object-contain' />
            </div>
        )): <h1>no data</h1>}
    </div>
    <div className='absolute top-0 bottom-0 w-full flex justify-between items-center px-5'>
